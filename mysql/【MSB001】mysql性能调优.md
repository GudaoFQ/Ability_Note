...
### 方法一：通过show profile语句查询sql执行性能信息【默认关闭；高版本以后可能会不支持，5版本以下的都可以用】
#### 步骤说明
1. show profile默认是禁用的，可以通过服务器变量在绘画级别动态的修改
   set profiling=1;
2. 当设置完成之后，在服务器上执行的所有语句，都会测量其耗费的时间和其他一些查询执行状态变更相关的数据。
   select * from emp;
3. 在mysql的命令行模式下只能显示两位小数的时间，可以使用如下命令查看具体的执行时间
   show profiles;
4. 执行如下命令可以查看详细的每个步骤的时间：
   show profile for query 1;

#### 操作实例
1. mysql连接成功后进入数据库<br>
   ![数据库切换+sql执行](../resource/mysql/mysql-数据库切换+sql执行.png)
2. 启用 `show profiles` 命令<br>
   ![show profile开启](../resource/mysql/mysql-show profile开启.png)
3. 再次执行sql语句，查看语句执行效率<br>
   ![查看sql执行效率](../resource/mysql/mysql-查看sql执行效率.png)
4. 查询指定执行语句（通过Query_ID）详细信息<br>
   ![show profile查询指定sql执行详情](../resource/mysql/mysql-show profile查询指定sql执行详情.png)

#### 其余操作命令介绍
![show profile其余命令说明](../resource/mysql/mysql-show profile其余命令说明.png)

### 方法二：通过performance schema监控sql语句执行【默认开启；官方推荐使用】
**MySQL的performance schema 用于监控MySQL server在一个较低级别的运行过程中的资源消耗、资源等待等情况**。

#### 位置说明
![performance schema位置说明](../resource/mysql/msyql-performance schema位置说明.png)
